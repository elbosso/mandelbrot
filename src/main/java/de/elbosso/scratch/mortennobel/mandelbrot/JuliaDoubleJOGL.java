...
public class JuliaDoubleJOGL extends GLCanvas implements GLEventListener, MandelbrotRender {
	...
	public JuliaDoubleJOGL(MandelbrotSetting settings) {
...
//		animator.setRunAsFastAsPossible(true);
	...
	private void attachShaders(GL2 gl) throws Exception {
		...
		String[] vsrc = loadShaderSrc("juliad2.vs");
		gl.glShaderSource(vertexShaderProgram, 1, vsrc, null, 0);
		gl.glCompileShader(vertexShaderProgram);

		String[] fsrc = loadShaderSrc("juliad2.fs");
	...
	private void updateUniformVars(GL2 gl) {
		...
		gl.glUniform2f(mandel_reC, settings.getReCAsFloat(),(float)(settings.getReC()-(double)settings.getReCAsFloat()));
		gl.glUniform2f(mandel_imC, settings.getImCAsFloat(),(float)(settings.getImC()-(double)settings.getImCAsFloat()));
