...
public class JuliaJava2DThreaded extends JComponent implements MandelbrotRender {
	...
	private de.netsysit.util.threads.ThreadManager threadManager=new de.netsysit.util.threads.ThreadManager(JuliaJava2DThreaded.class.getName(),Runtime.getRuntime().availableProcessors()+1);

	public JuliaJava2DThreaded(MandelbrotSetting settings) {
	...
	private void updateMandelbrotImage() {
		...
		int tiledim=128;
		int yoff=0;
		CubbyHole<MandelbrotTile> ch=new de.netsysit.util.threads.SimpleBufferingCubbyHole();
		while(yoff<image.getHeight())
		{
			int xoff=0;
			while(xoff<image.getWidth())
			{
				int w= Math.min(image.getWidth()-xoff,tiledim);
				//System.out.println("#"+image.getWidth()+" "+xoff+" "+tiledim);
				int h= Math.min(image.getHeight()-yoff,tiledim);
...
*/		stat.timerEnd();
	...
	{
		private CubbyHole<MandelbrotTile> ch;
		private int counter;
	...
	class MandelbrotTile extends de.elbosso.util.threads.StoppableImpl
	{
		...
		private final CubbyHole<MandelbrotTile> ch;

		public MandelbrotTile(int xoff, int yoff, int width, int height, double deltaX, double deltaY, int imageWidth, CubbyHole<MandelbrotTile> ch)
		{
			super();
